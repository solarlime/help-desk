...
export default class Page {
  ...
  addListeners() {
      ...
      cancel.addEventListener('click', (event) => {
        if (
          event.target
            ...
            .classList.contains('modal-add-update')
        ) {
        ...
        if (
          event.target
            .closest('.modal-container')
            .classList.contains('modal-delete')
        ) {
    ...
    this.form.querySelectorAll('input, textarea').forEach((input) => {
      input.addEventListener('input', (event) => {
        ...
        if (event.target.id === 'description') {
          this.validity.title = validation(
            this.page.querySelector('#title'),
            this.save,
          );
        }
      });
    ...
    this.form.addEventListener('input', () => {
      ...
      this.save.disabled = Object.values(this.validity).some(
        (input) => input === false,
      );
    });
  ...
  async quickSave(checkbox) {
    ...
    const target = this.list.data.find(
      (item) => item.id.toString() === row.getAttribute('data-id'),
    );
  ...
  render(item) {
    ...
    const htmlDescription = item.description.includes(enter)
      ? breakDescription(item.description)
      : item.description;
    ...
    listItemActions.insertAdjacentHTML(
      'beforeend',
